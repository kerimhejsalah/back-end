  ...
  try {
    ...
    if (!doctor) {
      ...
      if (doctor) {
        ...
        let savedDoctor = await Doctor.findByIdAndUpdate({ _id: doctor._id }, { password: hashedPassword })
        sendEmail(savedDoctor.email, password)
        return res.status(200).send({ etat: 'success' });
      }
    }
  } catch (error) {
    console.log(error);
